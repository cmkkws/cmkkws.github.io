...
여기서 $\alpha$는 prior의 precision이고, $\beta$는 noise의 precision이다. 따라서 $L_2$ regularization은 Bayesian 관점에서 Gaussian prior를 둔 MAP estimation으로 해석할 수 있다.`,{headingReplacements:{"polynomial curve fitting":"Polynomial Curve Fitting","1-D / Multivariate Gaussian distribution":"1차원·다변량 Gaussian 분포"}}),x(String.raw`# Binary Classification
...
| $dZ^{[1]}$        |    $64\times256$ |
| $dW^{[1]}$        |  $64\times49152$ |
| $db^{[1]}$        |      $64\times1$ |`,{headingReplacements:{"layer가 여러 개 일 때,":"여러 레이어일 때","LAYER 1(64 Nodes)":"Layer 1(64 Nodes)","LAYER 2(8 Nodes)":"Layer 2(8 Nodes)","LAYER 3(1 Node)":"Layer 3(1 Node)"}}),x(String.raw`## Train/dev/test sets
1) 데이터가 적은 경우
- 70/30% 또는 60/20/20%
...
입력 $x$만 정규화하는 것이 아니라 layer마다 새로 계산되는 $Z$를 mini-batch 기준으로 정규화한다.`,{headingReplacements:{}}),x(String.raw`다음은 K-means Clustering의 수식적인 설명과 Maximum Likelihood의 관계(가우시안 포함)를 MLE의 한계를 다룬다.
...
(MLE: cluster를 늘려 Loss를 줄이려고 하지만, 이는 noise까지 학습한 overfitting 상태가 됨)`,{headingReplacements:{"estimate $\\mu_k$":"μₖ 추정"}}),x(String.raw`다음은 1-D Gaussian 분포에 대한 MLE와 Bayesian 접근법의 차이에 대한 내용이다.
